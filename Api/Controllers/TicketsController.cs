...
public class TicketsController : ControllerBase
{
    ...
    public async Task<PagedOutputDto<TicketListOutputDto>> GetList([FromQuery] TicketListInputDto request)
    {
        ...
        {
            SortOptions.Asc => query.OrderBy(x => x.CreatedAt),
            _ => query.OrderByDescending(x => x.CreatedAt),
        };

        var projectedQuery = query.Select(x => new TicketListOutputDto(
        x.Id,
        x.Subject,
        x.Status,
        x.Customer.Name,
        ...
        return await projectedQuery.ToPagedResponse(request.Size, request.Page);
    }
